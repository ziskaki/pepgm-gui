...
rule AddDecoys:
    input: DatabaseDirectory + '{DBname,[A-Za-z]+}_UNI.fasta'
...
rule SearchSpectra:
    ...
    shell: 'cp config/config.yaml {params.ResultsDir}{params.samplename}/  &&  java -cp ' + str(SearchGUI) + ' eu.isas.searchgui.cmd.SearchCLI -spectrum_files {input[0]} -fasta_file {input[1]} -output_folder ' + str(ResultsDir) + '{params.samplename} -id_params {input[2]} -output_default_name {params.DBname}_searchgui_out -psm_fdr ' + str(psmFDR) + ' -peptide_fdr ' + str(peptideFDR) + ' -protein_fdr ' + str(proteinFDR) + ' ' + str(searchengines) + ' 1'
