...
class Messages():
        ...
        for node in CTGraphIn.nodes(data=True):
            if node[1]['category'] == 'factor':
                ...
                self.InitialBeliefs[node[0]] = np.ones(
                    4)  # this entry will never be used as the convolution trees do not hold beliefs
    ...
    def ComputeOutMessageVariable(self, NodeOUT, NodeIN):
        ...
        if not IncomingMessages:
            ...
            OutMessageLog = lognormalize(np.asarray([np.sum([np.log(NodeBelief[0]), np.sum(IncomingMessages[:, 0])]),
    ...
    def ComputeOutMessageFactor(self, NodeOUT, NodeIN):
        ...
        if self.Graph.nodes[NodeIN]['category'] == 'Convolution Tree':
            ...
            OutMessages = normalize(
                np.multiply(NodeBelief, [np.prod(IncomingMessages[:, 0]), np.prod(IncomingMessages[:, 1])]))
    ...
    def ComputeOutMessagesCTtree(self, Node):
        ...
        if all(OldSharedLikelihoods != sharedLikelihoods) and any(
                [(ProtProbList[i][0]) != (OldProtProbList[i][0]) for i in range(len(ProtProbList))]):
    ...
    def SingleEdgeDirectionUpdate(self, StartName, EndName):
        ...
        if self.Graph.nodes[StartName]['category'] == self.category or self.Graph.nodes[StartName][
            'category'] == 'peptide':
    ...
    def LoopyLoop(self, maxLoops, tolerance, local=False):
        ...
        for Variable in self.Graph.nodes():
            ...
            if self.Graph.nodes[Variable]['category'] == self.category or self.Graph.nodes[Variable][
                'category'] == 'peptide':
                ...
                IncomingMessages = np.asarray(np.log(IncomingMessages)).reshape(len(IncomingMessages), 2)
                LoggedVariableMarginal = lognormalize(np.asarray(
                    [np.sum([np.log(self.InitialBeliefs[Variable][0]), np.sum(IncomingMessages[:, 0])]),
                     np.sum([np.log(self.InitialBeliefs[Variable][1]), np.sum(IncomingMessages[:, 1])])]))
...
def SaveResultsToCsv(ResultsDict, NodeDict, NameString):
    ...
    pd.DataFrame.from_dict(data=FullResultsDict, orient='index').to_csv(NameString,
                                                                        header=False)  # (datetime.now().strftime("%Y-%-m-%d-%H-%M-%S")+'-' +NameString + '-Results.csv', header = False)
...
if __name__ == '__main__':
    ...
    CTFactorgraphs = [SeparateSubgraphs(CTFactorgraph, filternodes) for filternodes in
                      nx.connected_components(CTFactorgraph)]
